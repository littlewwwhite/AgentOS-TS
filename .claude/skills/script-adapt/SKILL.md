...
将体量充足的长篇小说原文（≥3000 字）快速改编为可指导 AI 生成画面的结构化微短剧剧本（抖音竖屏，单集 60 秒）。

- 本 skill **只处理长篇小说直转 / 简单改编**
- **不处理原创**
- **不处理短篇扩写 / NTSV2 / SWS**，这些由 `script-writer` 负责
...
进入本 skill 后，先读取 `references/route-table.md`，按其中维度做一次**解释型路由判断**。这个判断当前只用于说明“为什么是 `script-adapt`”，**不接管真实调度**。
...
- 若用户目标是**长篇直转 / 简单改编 / 保留原文骨架**，本 skill 继续执行
- 若用户输入是**已有结构化分镜 / 结构稿**，且目标是格式转换或结构提取，本 skill 继续执行
- 若用户目标明显是**原创**或**短篇扩写成长剧**，先明确指出更匹配 `script-writer`，不要在本 skill 内硬做
- 对于 **3000-10000 字重叠区间**，**不能按字数硬切**，必须按创作目标判断：保骨架走 `script-adapt`，要扩写走 `script-writer`

真正开始 Phase A/B 之前，先用 `references/route-table.schema.json` 的字段语义输出一次简短路由说明，至少包含：
...
| Script | Purpose | CLI |
|--------|---------|-----|
| `prepare_source_project.py` | 将任意格式文件（txt/md/docx/xlsx/pdf）转为 `${PROJECT_DIR}/source.txt`。返回 `mode` 字段：`storyboard`（docx 含嵌入 Excel，格式转换）或 `novel`（纯文本，创作改编） | `python3 ${CLAUDE_SKILL_DIR}/scripts/prepare_source_project.py --source-path <file> --workspace-path ${PROJECT_DIR}` |
| `detect_source_structure.py` | 检测原文章节/分集边界，写入 `${PROJECT_DIR}/draft/source-structure.json` | `python3 ${CLAUDE_SKILL_DIR}/scripts/detect_source_structure.py --project-path ${PROJECT_DIR} [--max-chars 8000]` |
| `parse_script.py` | 解析 ep*.md 为 script.json + 内置验证报告（结构完整性 + catalog 覆盖 + 双语合规）。`--validate` 时退出码反映验证状态 | `python3 ${CLAUDE_SKILL_DIR}/scripts/parse_script.py --project-path ${PROJECT_DIR} --output-path ${PROJECT_DIR}/output [--validate]` |
| `plan_phase2_batches.py` | 检查已完成集数，决定 Phase B 是否并行写作。从 `source-structure.json` 读取已检测分段，写入 `draft/source_packs/group_N.md`，返回 `groups[].source_pack_path` | `python3 ${CLAUDE_SKILL_DIR}/scripts/plan_phase2_batches.py --project-path ${PROJECT_DIR}` |
| `normalize_episodes.py` | **Phase B2 格式修复**：自动修正复合名、分隔符、缺失符号。输出统一 JSON 格式 | `python3 ${CLAUDE_SKILL_DIR}/scripts/normalize_episodes.py --project-path ${PROJECT_DIR}` |
...
|-------|--------------|--------------|
| **Route Gate** | `route-table.md` | route explanation only |
| **Phase A** | `phase1-design.md`, `shared-domain.md`, `style-options.md` | design.json + catalog.json |
| **Phase B** | `phase2-writing.md`, `shared-language-rules.md`, `writing-rules.md`, `script-format.md` | episodes/ep\*.md → script.json |
...
`${PROJECT_DIR}` 固定为当前项目根目录。所有中间产物写入 `${PROJECT_DIR}/draft/`，最终产物写入 `${PROJECT_DIR}/output/`。

### 工作区结构

```
${PROJECT_DIR}/                           <- 当前项目根目录
├── pipeline-state.json                <- 跨 skill 状态索引
├── source.txt                         <- 原文副本
├── draft/                            <- 中间产物（LLM 写入）
...
- **初始化**：收到原文后，确保 `${PROJECT_DIR}` 存在，保存原文至 `${PROJECT_DIR}/source.txt`
...
1. 检查 `${PROJECT_DIR}/draft/source-structure.json` → 存在则原文结构检测已完成
2. 检查 `${PROJECT_DIR}/draft/design.json` + `${PROJECT_DIR}/draft/catalog.json` → 全部存在则 **Phase A 完成**（到达 CRUD 检查点）
3. 检查 `${PROJECT_DIR}/draft/episodes/` 目录（至少一个 ep*.md）→ 存在则 Phase B 进行中/完成
4. 检查 `${PROJECT_DIR}/output/script.json` → 存在则 Phase B 完成
...
状态文件契约遵循 `docs/pipeline-state-contract.md`，本 skill 作为 `SCRIPT` 阶段的另一条入口时必须同步维护 `${PROJECT_DIR}/pipeline-state.json`。
...
- **初始化**：收到长篇原文并创建 `${PROJECT_DIR}/source.txt` 后，设置 `current_stage=SCRIPT`、`stages.SCRIPT.status=running`
- **Phase A 完成**：`source-structure.json`、`design.json`、`catalog.json` 就绪后，保持 `stages.SCRIPT.status=running`
- **Phase B 写作中**：已产生 `${PROJECT_DIR}/draft/episodes/ep*.md` 但尚未完成解析时，设置 `stages.SCRIPT.status=partial`
- **结构解析完成**：产出 `${PROJECT_DIR}/output/script.json` 后，设置 `stages.SCRIPT.status=completed`
- **门控通过**：`script.json` 解析验证无误后，设置 `stages.SCRIPT.status=validated`、`next_action=enter VISUAL`
...
2. 若缺失，再按本节上方的工作区文件顺序恢复
3. 文件扫描只用于补状态，不覆盖更新鲜的显式状态
...
| 阶段 | 输入 | 输出 | 工作区文件 |
|------|------|------|-----------|
| Phase A 分析设计 | `${PROJECT_DIR}/source.txt` | source-structure.json + design.json + catalog.json | `${PROJECT_DIR}/draft/*.json` |
| Phase B 写作 | `${PROJECT_DIR}/draft/source-structure.json` + `${PROJECT_DIR}/draft/design.json` + `${PROJECT_DIR}/draft/catalog.json` | ep\*.md → script.json | `${PROJECT_DIR}/draft/episodes/*.md` + `${PROJECT_DIR}/output/script.json` |

### 依赖矩阵

| 目标阶段 | 前置文件 |
|---------|---------|
| Phase A | `${PROJECT_DIR}/source.txt` |
| Phase B | `${PROJECT_DIR}/draft/design.json` + `${PROJECT_DIR}/draft/catalog.json`（source-structure.json 可选，用于原文定位） |
...
1. 调用 `python3 ${CLAUDE_SKILL_DIR}/scripts/prepare_source_project.py --source-path <file> --workspace-path ${PROJECT_DIR}` 转换为 `${PROJECT_DIR}/source.txt`
...
3. 调用 `python3 ${CLAUDE_SKILL_DIR}/scripts/detect_source_structure.py --project-path ${PROJECT_DIR}` 生成 `${PROJECT_DIR}/draft/source-structure.json`
4. 读取 `${PROJECT_DIR}/draft/source-structure.json` 的 `planning` 字段，按以下优先级决定分集：
...
1. 将交付物保存至对应工作区目录
2. **Phase A JSON 校验**：写入 design.json 和 catalog.json 后，立即执行 `python3 -m json.tool ${PROJECT_DIR}/draft/design.json > /dev/null && python3 -m json.tool ${PROJECT_DIR}/draft/catalog.json > /dev/null`。失败则修复 JSON 后重新写入
...
**触发前置检查**：先执行 `python3 ${CLAUDE_SKILL_DIR}/scripts/plan_phase2_batches.py --project-path ${PROJECT_DIR}`。
...
4. shared-language-rules.md + writing-rules.md + script-format.md 的格式规范
...
↓
Phase B2 格式修复：
  python3 ${CLAUDE_SKILL_DIR}/scripts/normalize_episodes.py --project-path ${PROJECT_DIR}
  → 自动修复格式问题（分隔符、缺失符号等）
  → 输出 fixes_applied 列表
↓
Phase B3 解析 + 内置验证：
  python3 ${CLAUDE_SKILL_DIR}/scripts/parse_script.py --project-path ${PROJECT_DIR} --output-path ${PROJECT_DIR}/output --validate
...
- Phase 3 通过 `python3 ${CLAUDE_SKILL_DIR}/scripts/parse_script.py --project-path ${PROJECT_DIR} --output-path ${PROJECT_DIR}/output` 执行解析
