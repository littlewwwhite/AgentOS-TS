...
用法（独立运行）:
  ...
  python3 auth.py --base-url https://animeworkbench.lingjingai.cn  # 指定 Base URL
"""
...
# 运行时 BASE_URL，可通过 set_base_url() 或环境变量 AWB_BASE_URL 覆盖
_base_url = os.environ.get("AWB_BASE_URL", DEFAULT_BASE_URL)


def set_base_url(url: str) -> None:
    """设置 API Base URL（供外部调用方在 import 后覆盖）。"""
    global _base_url
    _base_url = url
...
def _call_refresh_token(sid: str, last_token: str = None) -> tuple:
    ...
    req = urllib.request.Request(
        f"{_base_url}/api/anime/user/account/refreshToken",
        data=data,
        headers=headers,
        method="POST",
    )
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="打印当前用户信息和 token")
