...
def generate_video_with_anime_workbench(
    ...
    # video-create scripts (local copies for isolation)
    local_scripts_dir = Path(__file__).resolve().parent
    submit_script = local_scripts_dir / "submit_video_create.py"
    poll_script = local_scripts_dir / "poll_video_create_task.py"

    if not submit_script.exists():
        return {
            "success": False,
            "message": f"submit_video_create.py not found: {submit_script}",
            "video_path": None,
            "task_id": None
        }
