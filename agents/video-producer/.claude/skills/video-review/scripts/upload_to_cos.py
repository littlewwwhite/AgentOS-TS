#!/usr/bin/env python3
"""
upload_to_cos.py — 上传本地文件到腾讯云 COS（设置 public-read，返回可直接访问的 URL）

用法：
    python3 upload_to_cos.py --file /path/to/file.jpg
...
def get_cos_secret(base_url: str, token: str, group_id: str, scene_type: str) -> dict:
...
def upload_file(credential: dict, file_path: str, group_id: str) -> str:
    """上传文件到 COS（public-read），返回可直接访问的 plain URL。"""
    ...
    # path 可能带尾部斜杠，也可能不带
    cos_path = credential.get("path", f"material/video-create/{group_id}/")
    ...
    object_key = f"{cos_path}upload-{timestamp_ms}-{rand}-{safe_filename}"

    with open(file_path, "rb") as f:
        file_data = f.read()

    ext_str = raw_filename.rsplit(".", 1)[-1].lower() if "." in raw_filename else ""
    content_type = CONTENT_TYPE_MAP.get(ext_str, "application/octet-stream")

    host = f"{bucket}.cos.{region}.myqcloud.com"
    cos_url = f"https://{host}/{object_key}"
    uri_path = f"/{object_key}"

    # 上传时设置 x-cos-acl: public-read，使文件公开可读
    headers_to_sign = {
...
def main():
    parser = argparse.ArgumentParser(description="上传本地文件到腾讯云 COS")
    ...
    parser.add_argument("--scene-type", default="material-video-create",
                        help="COS 场景类型，默认 material-video-create（使用 --credentials-json 时忽略此参数）")
    ...
    if args.credentials_json:
        ...
        print(f"正在获取 COS 临时凭证（sceneType: {args.scene_type}）...")
        try:
            resp = get_cos_secret(args.base_url, token, group_id, args.scene_type)
