...
class VideoRegenerator:
    ...
    def detect_generator(self) -> str:
        """Detect available video generation tool (local scripts)."""
        local_dir = Path(__file__).resolve().parent

        # Check local submit_video_create.py (copied from video-create)
        if (local_dir / "submit_video_create.py").exists():
            return "video-create"
