#!/usr/bin/env python3
"""
poll_video_create_task.py — 轮询视频生成任务状态，直至完成或超时

用法：
    python3 poll_video_create_task.py --task-id <taskId> \
...
def poll(base_url: str, task_id: str,
...
def main():
    parser = argparse.ArgumentParser(description="轮询视频生成任务状态")
    ...
    parser.add_argument("--interval", type=int, default=5, help="轮询间隔（秒），默认 5")
    parser.add_argument("--timeout", type=int, default=600, help="超时时间（秒），默认 600")
    args = parser.parse_args()

    print(f"开始轮询视频生成任务: {args.task_id}")
    print(f"轮询间隔: {args.interval}s  超时: {args.timeout}s")
    print("-" * 50)

    sys.exit(poll(args.base_url, args.task_id,
