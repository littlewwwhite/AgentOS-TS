<!-- NOTE: scripts/ 目录下的脚本尚未实现，此 skill 当前为设计规范 -->
---
name: music-matcher
description: >
  基于向量语义匹配的智能视频配乐工具。
  当用户需要"给视频配乐"、"自动配乐"、"选音乐"、"匹配背景音乐"时使用。
  支持 Gemini 视频分析、向量匹配选曲、FFmpeg 合成输出。
allowed-tools:
  - Read
  - Bash
---
...
`PROJECT_DIR` 是运行时产物的根目录（data/、output/ 均写入此处），应由用户指定或从上下文推断（如当前工作目录）。`${CLAUDE_SKILL_DIR}` 仅用于定位 skill 自身的脚本文件。

```
# 示例：用户指定工作区
PROJECT_DIR = /path/to/your/project

# skill 脚本所在位置（只读资源，不写运行时产物）
SKILL_SCRIPTS_DIR = ${CLAUDE_SKILL_DIR}/scripts/
```

运行前请确认 `PROJECT_DIR` 已正确设置。
...
```bash
cd $PROJECT_DIR && python ${CLAUDE_SKILL_DIR}/scripts/embed_library.py
```
...
```bash
cd $PROJECT_DIR && python ${CLAUDE_SKILL_DIR}/scripts/analyze_video.py "<视频绝对路径>"
```
...
```bash
cd $PROJECT_DIR && python ${CLAUDE_SKILL_DIR}/scripts/match.py --top 5 --output output/results.json
```
...
```bash
cd $PROJECT_DIR && python ${CLAUDE_SKILL_DIR}/scripts/compose.py "<视频绝对路径>" output/results.json
```
...
```bash
cd $PROJECT_DIR && rm -f data/music_vectors.pkl && python ${CLAUDE_SKILL_DIR}/scripts/embed_library.py
```
...
1. **指定 rank**：用户说"片段 2 用第 3 名的曲子"
   ```bash
   cd $PROJECT_DIR && python ${CLAUDE_SKILL_DIR}/scripts/compose.py "<视频路径>" output/results.json --rank 3
   ```
