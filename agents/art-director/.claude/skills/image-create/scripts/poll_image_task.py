#!/usr/bin/env python3
"""
poll_image_task.py — 轮询图片生成任务状态，直至完成或超时

用法：
    python3 poll_image_task.py --task-id <taskId> [--interval 3] [--timeout 300]
...
def query_task(base_url: str, task_id: str) -> dict:
    url = f"{base_url}/api/material/creation/imageCreateGet?taskId={task_id}"
...
def poll(base_url: str, task_id: str, interval: int, timeout: int) -> int:
    start = time.time()
    attempt = 0
    ...
    while True:
        ...
        try:
            resp = query_task(base_url, task_id)
        ...
        if status in TERMINAL_STATUSES:
            ...
            if status == "SUCCESS":
                ...
                print(f"生成失败！错误信息：{data.get('errorMsg', '未知错误')}", file=sys.stderr)
                return 1
...
def main():
    parser = argparse.ArgumentParser(description="轮询图片生成任务状态")
    ...
    print(f"开始轮询任务: {args.task_id}")
    print(f"轮询间隔: {args.interval}s  超时: {args.timeout}s")
    print("-" * 50)

    sys.exit(poll(args.base_url, args.task_id, args.interval, args.timeout))
