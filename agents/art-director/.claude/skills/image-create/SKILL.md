...
description: 通过 anime-material-workbench API 生成图片，支持角色、场景、道具等图片的创建与异步任务管理。当用户提到"生成图片"、"出图"、"画一张"、"生成角色/场景/道具图"、"调用生图API"时使用。不适用于批量生成整个项目资产（使用 asset-gen）或图片编辑修改（使用 image-edit）。
---
...
- **API 文档**：`${CLAUDE_SKILL_DIR}/references/api.md` — 完整的接口规范、字段说明和示例
...
```bash
python3 ${CLAUDE_SKILL_DIR}/scripts/submit_image_create.py \
...
python3 ${CLAUDE_SKILL_DIR}/scripts/poll_image_task.py \
  --task-id "TASK_ID"
```
...
- Always load `${CLAUDE_SKILL_DIR}/references/api.md` when looking up field details, error codes, or full examples.
