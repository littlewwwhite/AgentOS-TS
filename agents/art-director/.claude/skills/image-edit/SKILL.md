...
description: 通过 anime-material-workbench API 编辑图片，支持图片修改、风格迁移等编辑操作与异步任务管理。当用户提到"修改图片"、"编辑图"、"图片风格迁移"、"调整图片"、"重绘"时使用。不适用于从零生成新图片（使用 image-create）或批量资产生成（使用 asset-gen）。
---
...
```bash
python3 ${CLAUDE_SKILL_DIR}/scripts/submit_image_edit.py \
...
python3 ${CLAUDE_SKILL_DIR}/scripts/poll_image_edit_task.py \
  --task-id "TASK_ID"
```
