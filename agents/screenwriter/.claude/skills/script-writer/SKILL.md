...
- **工作流指引**（${CLAUDE_SKILL_DIR}/script-writer-references/）：`${CLAUDE_SKILL_DIR}/script-writer-references/` — 用 `Read` 加载
...
**加载顺序**：工作流指引（${CLAUDE_SKILL_DIR}/script-writer-references/）→ 共享知识（resources/）→ 输出模板（assets/，输出交付物时才读取）

| 阶段 | 工作流指引（${CLAUDE_SKILL_DIR}/script-writer-references/） | 共享知识（resources/） | 输出模板（assets/） |
|------|--------------------------|----------------------|----------------------|
...
详见 `${CLAUDE_SKILL_DIR}/script-writer-references/orchestration.md`（工作区结构、操作、上下文恢复、启动流程、阶段流转、状态查询、跳转逻辑、用户输入路由）。
