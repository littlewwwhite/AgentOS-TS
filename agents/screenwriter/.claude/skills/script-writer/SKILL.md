...
- **工作流指引**（references/）：`${CLAUDE_SKILL_DIR}/script-writer-references/` — 用 `Read` 加载
- **共享知识**（resources/）：`${CLAUDE_SKILL_DIR}/script-writer-resources/` — 用 `Read` 加载
- **输出模板**（assets/）：`${CLAUDE_SKILL_DIR}/script-writer-assets/` — 输出交付物时用 `Read` 加载
