---
name: script-writer
description: 微短剧剧本写作流水线：支持原创（SWS）和改编扩写（NTSV2）两种模式，通过九阶段流程产出可指导 AI 生成画面的结构化剧本。当用户提到原创剧本、剧本写作、SWS、小说改编、小说扩写、NTSV2 时使用。不适用于仅需快速三阶段直转剧本的项目，此时改用 script-adapt。
allowed-tools:
  - Read
  - Write
  - Bash
...
支持两种创作模式，通过九阶段流程产出可指导 AI 生成画面的抖音竖屏微短剧剧本（单集 60 秒）。
...
| 原文结构检测 | 不适用 | S1 前置步骤，生成 `draft/source-structure.json` |
...
- **工作流指引**（${CLAUDE_SKILL_DIR}/references/）：`${CLAUDE_SKILL_DIR}/references/` — 用 `Read` 加载
- **共享知识**（resources/）：`${CLAUDE_SKILL_DIR}/resources/` — 用 `Read` 加载
- **输出模板**（assets/）：`${CLAUDE_SKILL_DIR}/assets/` — 输出交付物时用 `Read` 加载
- **确定性脚本**（scripts/）：`${CLAUDE_SKILL_DIR}/scripts/` — 用 `Bash` 调用

### Bundled Scripts

| Script | Purpose | CLI |
|--------|---------|-----|
| `detect_source_structure.py` | NTSV2 模式 S1 前置：检测原文章节边界，写入 `draft/source-structure.json` | `python3 ${CLAUDE_SKILL_DIR}/scripts/detect_source_structure.py --project-path <dir> [--max-chars 8000]` |
...
**加载顺序**：工作流指引（${CLAUDE_SKILL_DIR}/references/）→ 共享知识（resources/）→ 输出模板（assets/，输出交付物时才读取）

| 阶段 | 工作流指引（${CLAUDE_SKILL_DIR}/references/） | 共享知识（resources/） | 输出模板（assets/） |
|------|--------------------------|----------------------|----------------------|
...
| **S7** 逐层写作 | `s7-writing.md` | 详见 `s7-writing.md` 输入检查清单（含 Skill 内置规则 + 工作区过程文件完整列表） | `s7-deliverables.md` |
| **S8** 润色终审 | `s8-polish.md` | `writing-rules.md` + `{workspace}/style-guide.md` + `{workspace}/anchor.md` + `{workspace}/s7-scripts.md` | `s8-deliverables.md` |
...
- ^按需①^ `genre-database.md`：当用户的创意输入涉及非常规题材组合、跨类型融合、或需要确认题材标签归属时加载
- ^按需②^ `trope-library.md`：当需要查找特定叙事套路（trope）的标准结构、或评估原文中套路的变体利用方式时加载

---

## 工作区管理与编排逻辑

详见 `${CLAUDE_SKILL_DIR}/references/orchestration.md`（工作区结构、操作、上下文恢复、启动流程、阶段流转、状态查询、跳转逻辑、用户输入路由）。
