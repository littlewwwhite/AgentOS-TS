...
## 2. Skills — Agent 工作区约定

当前仓库将领域技能文件放在 `agents/<name>/.claude/skills/*.md`，并通过 agent 自身的 `cwd` + `settingSources: ["project"]` 让 Claude 读取该 agent 工作区下的 `.claude/` 配置。

需要区分两层语义：

- **SDK 明确文档化的部分**：`settingSources: ["project"]` 会读取当前 `cwd` 下的 `.claude/settings.json` 与 `CLAUDE.md`
- **本仓库的工作区约定**：领域技能文件与引用资料同样存放在 `.claude/skills/` 中，agent 通过上下文和 `Read` 工具按需读取它们

因此，这里不再把 `.claude/skills/*.md` 表述为 SDK reference 明确保证的“原生技能注入”能力，而是项目侧采用的技能组织方式。
...
SDK 启动 sub-agent 时，以 `agents/<name>/` 为 `cwd`，设置 `settingSources: ["project"]`，会读取当前 agent 工作区下的 `.claude/` 配置：
...
- **skills/*.md** — 本仓库用于存放领域技能说明与引用资料的目录，供 agent 在该工作区内消费
...
| 维度 | 旧架构 | 新架构 |
|------|--------|--------|
| 知识来源 | `skills/*/SKILL.md` → `loadSkillContents()` → `buildAgents()` 拼接 | `agents/<name>/.claude/skills/*.md`，按 agent 工作区约定组织 |
...
| 角色 | cwd | 配置来源 | 权限控制 |
|------|-----|---------|---------|
| Orchestrator | workspace/ | `options.ts` systemPrompt | 仅调度所需的 `TodoWrite` + `mcp__switch__switch_to_agent`，并显式 deny `Bash`/`Write`/`Edit` |
| Sub-agent | agents/\<name\>/ | `.claude/CLAUDE.md` + `settings.json` + `.claude/skills/` | 当前代码同时叠加 `permissionMode: "bypassPermissions"` 与 agent 自身 MCP/文件权限 |

**关键区别**：
- Orchestrator 的 `settingSources: ["project"]` 读取项目根 `.claude/`，权限主要由 `options.ts` 中的程序化 `allowedTools` / `disallowedTools` 控制
- Sub-agent 的 `settingSources: ["project"]` 读取 `agents/<name>/.claude/`，但当前实现还会在代码里额外设置 `permissionMode: "bypassPermissions"`，因此不是纯粹由 `settings.json` 单独控制
